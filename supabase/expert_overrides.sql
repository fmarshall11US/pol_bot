...
-- Allows users to override AI answers with expert corrections
...
CREATE TABLE IF NOT EXISTS expert_overrides (
  ...
  expert_explanation TEXT, -- Why the correction was needed
  expert_id TEXT NOT NULL, -- User who created the override
  
  -- Embedding for semantic matching
  question_embedding vector(1536),
  
  -- Metadata
  confidence_threshold FLOAT DEFAULT 0.85, -- How similar a question needs to be to trigger this override
  is_active BOOLEAN DEFAULT true,
  applies_to_all_documents BOOLEAN DEFAULT false, -- If true, applies globally
  document_ids UUID[] DEFAULT '{}', -- Specific documents this applies to
...
CREATE INDEX idx_expert_overrides_active ON expert_overrides(is_active);
CREATE INDEX idx_expert_overrides_embedding ON expert_overrides USING ivfflat (question_embedding vector_cosine_ops) WITH (lists = 100);
CREATE INDEX idx_override_usage_override_id ON override_usage(override_id);
CREATE INDEX idx_override_usage_created_at ON override_usage(created_at);
CREATE INDEX idx_override_versions_override_id ON override_versions(override_id);
